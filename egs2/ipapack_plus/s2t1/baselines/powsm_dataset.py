...
class PowsmDataset(Dataset):
    def __init__(self, wav_scp_path, text_phoneme_path, language_path, sampling_rate=16000):
        ...
        assert set(self.wav_scp.keys()).issubset(set(self.text.keys())), "Extra key in wav.scp"
        self.keys = list(self.wav_scp.keys())
        # print(self.keys[:10], list(self.key2lang.keys())[:10])
        assert all(k in self.key2lang for k in self.keys), "Missing language tags for some keys"
        print(f"Loaded dataset: {len(self.key2lang)} lang keys, {len(self.keys)} samples")
    ...
    def _load_text(self, path):
        ...
        with open(path) as f:
            for line in f:
                parts = line.strip().split()
                if len(parts) >= 2:
                    text_dict[parts[0]] = ' '.join(parts[1:]) # some examples have spaces in between we must retain full length of transcript
        return text_dict
    ...
    def _extract_language(self, path):
        ...
        with open(path) as f:
            for line in f:
                ...
                if key.endswith('_pr'): key = key[:-3] # remove _pr suffix for some datasets. This should be modified at source. It is bad design.
    ...
    def __getitem__(self, idx):
        ...
        return {"key": key, "wav": waveform, "transcription": transcription, "wavpath": wav_path, 'language': self.key2lang[key]}
...
def get_inference_dataset(dataset_name, dataset_config_path="dataset_config.yaml", **kwargs):
...
if __name__ == "__main__":
    ...
    item = dataset[0]
    print(f"Sample: {item['key']}, shape: {item['wav'].shape}, text: {item['transcription']}")